<script lang="ts">
	...
	function hasValidCoordinates(branch: Branch): boolean {
		return (
			Number.isFinite(branch.lat) &&
			Number.isFinite(branch.lng) &&
			Math.abs(branch.lat) <= 90 &&
			Math.abs(branch.lng) <= 180 &&
			(branch.lat !== 0 || branch.lng !== 0)
		);
	}

	function outletPriority(outletType: Branch['outletType']): number {
		switch (outletType) {
			case 'central':
				return 0;
			case 'branch':
	...
	function pickRepresentativeBranch(candidates: Branch[]): Branch {
		return [...candidates].sort((a, b) => {
			const outletDiff = outletPriority(a.outletType) - outletPriority(b.outletType);
			if (outletDiff !== 0) return outletDiff;

			const statusDiff = Number(a.status !== 'open') - Number(b.status !== 'open');
	...
	const mappableBranches = $derived(
		(() => {
			const bySystem: Record<string, Branch[]> = {};

			for (const branch of branches) {
				if (!hasValidCoordinates(branch)) continue;
				const systemBranches = bySystem[branch.systemId] ?? [];
				systemBranches.push(branch);
				bySystem[branch.systemId] = systemBranches;
			}

			return Object.values(bySystem).map((systemBranches) =>
				pickRepresentativeBranch(systemBranches)
			);
		})()
	);
...
<div class="map-page">
	<div class="controls">
		...
		<div class="legend">
			...
			<span class="legend-count">{mappableBranches.length} systems</span>
		</div>
