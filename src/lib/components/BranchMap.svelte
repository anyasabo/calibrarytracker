<script lang="ts">
	...
	function buildPopupHtml(branch: Branch): string {
		...
		return `
			<div class="branch-popup">
				...
				${system ? `<a href="${base}/systems/${system.id}" class="popup-system">${system.name}</a>` : ''}
	...
	function getFilteredBranches(): Branch[] {
		return branches.filter((b) => {
			if (!hasValidCoordinates(b)) return false;
	...
	async function initMap(): Promise<void> {
		...
		const clusterOptions: import('leaflet.markercluster').MarkerClusterGroupOptions = {
			maxClusterRadius: 50,
			chunkedLoading: true,
		...
		const LWithCluster = L as typeof L & {
			markerClusterGroup?: (
				options?: import('leaflet.markercluster').MarkerClusterGroupOptions
			) => import('leaflet.markercluster').MarkerClusterGroup;
			MarkerClusterGroup?: new (
				options?: import('leaflet.markercluster').MarkerClusterGroupOptions
			) => import('leaflet.markercluster').MarkerClusterGroup;
		};

		const markerClusterModule = markerCluster as {
			default?: {
				markerClusterGroup?: typeof LWithCluster.markerClusterGroup;
				MarkerClusterGroup?: typeof LWithCluster.MarkerClusterGroup;
			};
			markerClusterGroup?: typeof LWithCluster.markerClusterGroup;
			MarkerClusterGroup?: typeof LWithCluster.MarkerClusterGroup;
		...
		const markerClusterFactory =
			LWithCluster.markerClusterGroup ??
			markerClusterModule.markerClusterGroup ??
			markerClusterModule.default?.markerClusterGroup;
		const markerClusterCtor =
			LWithCluster.MarkerClusterGroup ??
			markerClusterModule.MarkerClusterGroup ??
			markerClusterModule.default?.MarkerClusterGroup;

		const cg = markerClusterFactory
			? markerClusterFactory(clusterOptions)
			: markerClusterCtor
				? new markerClusterCtor(clusterOptions)
				: undefined;
		if (!cg) {
			throw new Error('Failed to initialize leaflet.markercluster');
		}
